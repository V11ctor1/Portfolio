...
@app.route('/error404')
def well():  # колодец
    return render_template('well.html', title='<ERROR 404> Вот это поворот! Никогда не было и вот опять!',
                           year_app='Никогда', b='/index')


@app.route('/works')
def works():
    return render_template('works.html', title='Ведутся работы', year_app='В процессе', b='/index')
